...
type transport interface {
	self() *enode.Node
	ping(enode.ID, *net.UDPAddr,*snet.Addr) error
	findnode(toid enode.ID, addr *net.UDPAddr, saddr *snet.Addr, target encPubkey) ([]*node, error)
	close()
}
...
//no scionfied version needed due to new localnode struct
...
func (tab *Table) findnode(n *node, targetKey encPubkey, reply chan<- []*node) {
	...
	//SCION
	sAddr, serr := snet.AddrFromString(n.SCIONAddr())
	if serr != nil {
		log.Trace("SCION failed converting", n.SCIONAddr(), "to Addr", serr)
	}

	fails := tab.db.FindFails(n.ID())
	r, err := tab.net.findnode(n.ID(), n.addr(), sAddr, targetKey)
	if err != nil || len(r) == 0 {
		fails++
...
func (tab *Table) loadSeedNodes() {
	...
	for i := range seeds {
		...
		//fmt.Println("Found seed node in database", "id", seed.ID(), "addr", seed.addr(), "age", age)
		tab.add(seed)
	}
}
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	saddr, serr := snet.AddrFromString(last.SCIONAddr())
	if serr != nil {
		log.Trace("SCION failed converting", last.SCIONAddr(), "to Addr", serr)
	}

	// Ping the selected node and wait for a pong.
	err := tab.net.ping(last.ID(), last.addr(),saddr)
