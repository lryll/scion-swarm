...
func cmdLineOverride(currentConfig *bzzapi.Config, ctx *cli.Context) *bzzapi.Config {
	...
	if ctx.GlobalIsSet(utils.SCIONFlag.Name) {
		currentConfig.ScionEnabled = true
		if sAddr, err := snet.AddrFromString(ctx.GlobalString(utils.SCIONFlag.Name)); err == nil {
			currentConfig.SCIONAddr = sAddr.String()
		}
	}

	if ctx.GlobalIsSet(utils.SCIONDaemonPathFlag.Name) {
		currentConfig.SCIONDaemonPath = ctx.GlobalString(utils.SCIONDaemonPathFlag.Name)
	}

	if ctx.GlobalIsSet(utils.BootnodesFlagSCION.Name) {
		currentConfig.BootNodesSCION =  ctx.GlobalString(utils.BootnodesFlagSCION.Name)
	}

	if  ctx.GlobalIsSet(utils.SCIONTestRunFlag.Name) {
		currentConfig.SCIONTestRun = ctx.GlobalBool(utils.SCIONTestRunFlag.Name)
		scionutils.TestRunScionSwarm = ctx.GlobalBool(utils.SCIONTestRunFlag.Name)
		fmt.Println("SCION Test run flag set.")
	}

	if  ctx.GlobalIsSet(utils.CustomStatsPathFlag.Name) {

		scionutils.CustomStatsLogFilePath = ctx.GlobalString(utils.CustomStatsPathFlag.Name)
		fmt.Printf("ChunkStat dst file path set to%s\n",scionutils.CustomStatsLogFilePath)
	}
