...
func (d *Delivery) handleRetrieveRequestMsg(ctx context.Context, sp *Peer, req *RetrieveRequestMsg) error {
	...
	//remote := strings.Split(sp.Node().SCIONAddr(), "]:")[0] + "]"
	//local := strings.Split(sp.LocalAddr().String(),"]:")[0] + "]"


	//fmt.Printf("handleChunkDeliveryMsg local: %s\nremote: %s ", local,remote)

	handleRetrieveRequestMsgCount.Inc(1)
	//scionutils.CustomStatsRetrieveRequestMsgCount(local, remote)
...
func (d *Delivery) handleChunkDeliveryMsg(ctx context.Context, sp *Peer, req *ChunkDeliveryMsg) error {
	...
	//scionutils.CustomStatsReceiveChunkDelivery(local,remote)
